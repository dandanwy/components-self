    ...
    <div id="mQuery">
        <el-row>
            <el-col :span="config.showCollapse ? 22 : 24">
                <el-form ref="formData" :inline="config.inlineType" :model="config" :style="{maxHeight: collapse[collapseType].maxHeight, overflow: collapse[collapseType].overflow}" class="demo-form-inline">
                    <el-row v-for="(rowItem, rowIndex) in config.queryData.rows" :key="'row_'+rowIndex">
                        <el-col v-for="(item, index) in config.queryData.items" v-if="item.layout.row===rowItem" :key="'col_' + index" :span="item.layout.col">
                            <el-form-item :label="item.labelName" :prop="'queryData.items.' + index + '.labelValue'">
                    ...
                    <el-row type="flex" :justify="(config.buttons&&config.buttons.justify) || 'center'">
                        <el-col :span="6">
                            ...
                            <el-button type="ghost" @click="handleReset">{{(config.buttons&&config.buttons.resetText) || '重 置'}}</el-button>
                        </el-col>
                    </el-row>
                </el-form>
            </el-col>
        ...
        methods: {
            ...
            getCondition(config) {
                ...
                this.config.queryData.items.map((item, index) => {
                    ...
                    // if (item.labelValue && (item.labelValue !== '' || item.labelValue.length !== 0)) {
                        if (item.type === 'DatePicker' || item.type === 'TimePicker') {
                        if (item.pickerType === 'daterange' || item.pickerType === 'datetimerange') {
                            item.labelValue[0] = item.labelValue[0] ? parseTime(item.labelValue[0], item.format) : '';
                            item.labelValue[1] = item.labelValue[1] ? parseTime(item.labelValue[1], item.format) : '';
                        } else {
                            item.labelValue = item.labelValue ? parseTime(item.labelValue, item.format) : '';
                        }
                        }
                        arr += '"' + item.labelKey + '":"' + item.labelValue + '",';
                    // }
                });
                if (arr === '{') return {};
